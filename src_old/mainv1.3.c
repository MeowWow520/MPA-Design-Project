...
// v1.3: 精简版 (无旋转/无得分/无加速), 仅保留核心下落+消行
//

#include "REG52.H"

#define FALL_INTERVAL 20  // 下落间隔 (×50ms), 20=1s
...
const uint8 code piece_pattern[4][3] = {
    {0x80, 0x80, 0xC0},  // 0: L 形
    {0xC0, 0x60, 0x00},  // 1: Z 形
    {0x80, 0xC0, 0x80},  // 2: T 形
    {0xC0, 0xC0, 0x00}   // 3: O 形
};
const uint8 code piece_rows[4]   = {3, 2, 3, 2};
const uint8 code piece_br_col[4] = {1, 2, 0, 1}; // bottom-right 在底行中的列偏移

/* ── 全局变量 ── */
uint8 display_buffer[16] = {
    0x00, 0x00, 0x00, 0x00,   // 上屏 0..7
    0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00,   // 下屏 8..15
    0x00, 0x00, 0x00, 0x00
};

uint8  piece_type     = 0;             // 当前方块 0~3
uint8 *current_block  = NULL;          // → display_buffer[行]
uint8  piece_col      = 3;             // bottom-right 列 (0=bit7, 7=bit0)

uint8  scan_row       = 0;             // 当前扫描行 0~15
uint8  key_state      = 4;             // 0=LEFT 1=RIGHT 2=DOWN 3=START 4=无

bit    flag_fall      = 0;             // 重力到达
bit    flag_gameover  = 0;             // 游戏结束
bit    flag_pause     = 0;
...
/* ═══════════════════════════════════════════════
   定时器初始化: T0 显示扫描 1ms, T1 重力 50ms
   ═══════════════════════════════════════════════ */
void Timer_Init(void) {
    TMOD  = 0x11;           // T0 模式1, T1 模式1
...
/* ═══════════════════════════════════════════════
   HC595 发送 16 位 (MSB 优先, 两片级联)
   接线: MCU→595#1→595#2
         595#1 Q0~Q7 → 上屏 ROW0~7 (scan_row 0~7)
         595#2 Q0~Q7 → 下屏 ROW0~7 (scan_row 8~15)
   ═══════════════════════════════════════════════ */
void HC595_Send16(uint16 dat) {
    uint8 i;
    for (i = 0; i < 16; i++) {
...
bit CanPlace(uint8 *row_ptr, uint8 col, uint8 type) {
    ...
    for (i = 0; i < rows; i++) {
        ...
        if (expanded > 0xFF)  return 0;         // 列越界
        if (*check_row & (uint8)expanded) return 0;  // 碰撞
    }
    return 1;
}
